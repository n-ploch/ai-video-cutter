...
interface Props {
  ...
  isRunning: boolean
  onCreateNew: () => void          // clear state, show blank chat
...
export default function StoryboardVersionSidebar({
  versions,
  selectedVersion,
  isRunning,
  onCreateNew,
  ...
  return (
    <div className="w-52 flex flex-col border-r border-border bg-bg-secondary shrink-0 min-h-0">
      ...
      <div className="px-3 pt-3 pb-2">
        {isRunning ? (
          // While generating: gray button, clicking returns to active view
          <button
            onClick={() => onSelectVersion(null)}
          ...
          // Idle: accent button to start a new storyboard
          <button
            onClick={onCreateNew}
      ...
      <div className="flex-1 overflow-y-auto px-2 pb-2 space-y-0.5">
        ...
        {sorted.length === 0 && !isRunning && (
          <p className="text-xs text-muted px-2 py-3 text-center">No versions yet</p>
        )}
      </div>
    </div>
  )
