...
# Camera intrinsics
# ---------------------------------------------------------------------------
def estimate_intrinsics(frame_shape: tuple, fov_degrees: float = 84.0) -> np.ndarray:
    """Estimate camera matrix K from frame shape and horizontal FOV."""
    H, W = frame_shape[:2]
...
class CameraTrajectoryPipeline:
    ...
    def analyze(self, video_path: str, output_path: str) -> dict:
        ...
        for frame_rgb, timestamp, _ in tqdm(
            ...
            if K is None:
                K = estimate_intrinsics(frame_rgb.shape, cfg.fov_degrees)
                log.info(f"Intrinsics K:\n{K}")

            if prev_frame is not None:
                pts0, pts1 = self.matcher.match(prev_frame, frame_rgb)
                n_matches = len(pts0)
                pose = estimate_pose(pts0, pts1, K, cfg.min_matches)
            else:
                n_matches = 0
                pose = None
...
def main():
    ...
    parser.add_argument("--fov",         type=float, default=84.0,
                        help="Camera horizontal FOV in degrees (default: 84)")
